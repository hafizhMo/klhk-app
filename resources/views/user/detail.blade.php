...
@section('content')
    ...
    <object data="data:application/pdf;base64,{{ $attachment }}"
        ...
        <div>
            ...
            @if ($user->role !== 'user')
            <form action="{{ url('/user/detail-file/'. $id .'/' . $filename . '/approve?status=ditolak') }}" method="post">
                @csrf
                <button
                    class="mt-8 text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-sm px-12 py-2.5">Tolak</button>
            </form>
            <form action="{{ url('/user/detail-file/'. $id .'/' . $filename . '/approve?status=diterima') }}" method="post">
                @csrf
                <button
